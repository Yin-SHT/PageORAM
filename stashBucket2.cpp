...
void pageReadSubtree4(Node **a, int level, std::vector<std::pair<int,int>> &stash, int *pos, int value, int *numberOfNodesPerLevel, int *bucketSize)
{   
    // for(int i = 0; i < stash.size(); i++)
    // {
    //     if(stash[i].second == value)
    ...
    evict4(a, temp, level, stash, pos, numberOfNodesPerLevel, bucketSize);
    //PRINT STASH AFTER EVICTION
    // std::cout << "Stash after eviction is: ";
    // for(int i = 0; i < stash.size(); i++)
    // {
...
void pageDummyReadSubtree4(Node **a, int level, std::vector<std::pair<int,int>> &stash, int *pos, int value, int *numberOfNodesPerLevel, int *bucketSize)
{   
...
{   
    ...
    std::cout << "Enter the configuration (0 for PageORAM, 1 for PathORAM, 2 for PageORAM with subtree of height 4): ";
    int baseline;
    std::cin >> baseline;
    std::cout << std::endl;
    if(baseline == 0)
    ...
    if(baseline == 2)
        std::cout << "The configuration chosen is PageORAM with subtree of height 4" << std::endl;
    ...
    while(fin)
    {
        ...
        if(baseline == 2)
            pageReadSubtree4(tree, numOfLevels, stash, pos, stoi(a), numberOfNodesPerLevel, bucketSize);
        ...
        if(stash.size() > upperThreshold)
        {
            while(stash.size() > lowerThreshold){
                ...
                if(baseline == 2)
                    pageDummyReadSubtree4(tree, numOfLevels, stash, pos, ad, numberOfNodesPerLevel,  bucketSize);
